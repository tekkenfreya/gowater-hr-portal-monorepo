...
**IMPORTANT — Zod validation:** Both `/api/leads` and `/api/cold-leads` POST routes validate with `createLeadSchema` from `src/lib/validation/schemas.ts`. Zod strips unknown fields silently. Any new field added to `LeadFormData` MUST also be added to the schema, or it will be removed before reaching the service layer. `cold_category` is included in the `baseLeadSchema` as `coldCategorySchema.optional()`.
