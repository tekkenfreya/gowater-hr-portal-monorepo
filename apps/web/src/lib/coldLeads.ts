...
export class ColdLeadService {
  ...
  async createLead(employeeName: string, leadData: LeadFormData): Promise<Lead> {
    ...
    // Exclude hot_category — the cold_leads table does not have this column
    const { hot_category: _, ...dbLead } = lead;
    await this.db.insert('cold_leads', dbLead as unknown as Record<string, unknown>);
  ...
  async getLeadById(leadId: string): Promise<Lead | null> {
    const lead = await this.db.get('cold_leads', { id: leadId });
  ...
  async deleteLead(leadId: string): Promise<void> {
    await this.db.delete('cold_lead_activities', { lead_id: leadId });
    await this.db.delete('cold_leads', { id: leadId });
  }
  ...
  async logActivity(leadId: string, employeeName: string, activityData: ActivityFormData): Promise<LeadActivity> {
    ...
    await this.db.insert('cold_lead_activities', activity as unknown as Record<string, unknown>);

    // Update lead's updated_at timestamp
    await this.db.update('cold_leads', { updated_at: new Date().toISOString() }, { id: leadId });

    // If status update is provided, update lead status
    if (activityData.status_update) {
      await this.db.update('cold_leads', { status: activityData.status_update }, { id: leadId });
    }
  ...
  async getActivitiesForLead(leadId: string): Promise<LeadActivity[]> {
    const activities = await this.db.all('cold_lead_activities', { lead_id: leadId }, 'created_at');
    return (activities || []) as LeadActivity[];
  }

  async getAllActivities(): Promise<LeadActivity[]> {
    const activities = await this.db.all('cold_lead_activities', {}, 'created_at');
    return (activities || []) as LeadActivity[];
  }

  async deleteActivity(activityId: string): Promise<void> {
    await this.db.delete('cold_lead_activities', { id: activityId });
  }
  ...
  async getLeadsWithActivities(category?: LeadCategory, coldCategory?: string): Promise<LeadWithActivities[]> {
    ...
    const sql = `
      ...
      FROM cold_leads l
      LEFT JOIN cold_lead_activities la ON l.id = la.lead_id
      ${whereClause}
      GROUP BY l.id
...
export function getColdLeadService(): ColdLeadService {
  return new ColdLeadService();
}
