...
import type { LeadCategory, ColdCategory } from '@/types/leads';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (hotCategoryParam && !VALID_COLD_CATEGORIES.includes(hotCategoryParam as ColdCategory)) {
      return NextResponse.json(
        { error: `Invalid hot_category. Must be one of: ${VALID_COLD_CATEGORIES.join(', ')}` },
        { status: 400 }
      );
    }
    ...
    const filename = category
      ...
      : `all-hot leads-export-${timestamp}.xlsx`;
    ...
    logger.error('Export hot leads error', error);
    return NextResponse.json(
      { error: 'Failed to export hot leads data' },
      { status: 500 }
    );
  }
}
