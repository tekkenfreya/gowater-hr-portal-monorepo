...
export async function POST(request: NextRequest) {
  try {
    ...
    const activity = await coldLeadService.logActivity(leadId, employeeName, activityData as ActivityFormData);
    ...
    logger.error('Error logging cold lead activity', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (leadId) {
      const activities = await coldLeadService.getActivitiesForLead(leadId);
      return NextResponse.json({ activities, message: 'Activities fetched successfully' });
    } else {
      const activities = await coldLeadService.getAllActivities();
      return NextResponse.json({ activities, message: 'All activities fetched successfully' });
    }
  } catch (error) {
    logger.error('Error fetching cold lead activities', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const coldLeadService = getColdLeadService();
    await coldLeadService.deleteActivity(activityId);

    return NextResponse.json({ message: 'Activity deleted successfully' });
  } catch (error) {
    logger.error('Error deleting cold lead activity', error);
    return NextResponse.json(
